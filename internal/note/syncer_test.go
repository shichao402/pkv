...
func TestResolveNoteTargetDefaultsToItemName(t *testing.T) {
	item := types.Item{Name: "config/app.toml"}
	got, err := resolveNoteTarget(item, "team-a")
	if err != nil {
		t.Fatalf("resolveNoteTarget() error = %v", err)
	}
	if got != "config/app.toml" {
		t.Fatalf("resolveNoteTarget() = %q", got)
	}
}

func TestResolveNoteTargetUsesMiseConfDDefault(t *testing.T) {
	item := types.Item{
		Name:   "ruby toolchain.toml",
		Fields: []types.CustomField{{Name: types.PKVNoteStrategyFieldName, Value: types.NoteStrategyMiseConfD}},
	}
	got, err := resolveNoteTarget(item, "team prod")
	if err != nil {
		t.Fatalf("resolveNoteTarget() error = %v", err)
	}
	if got != filepath.Join(".config", "mise", "conf.d", "pkv-team-prod-ruby-toolchain.toml") {
		t.Fatalf("resolveNoteTarget() = %q", got)
	}
}

func TestResolveNoteTargetUsesExplicitTargetOverride(t *testing.T) {
	item := types.Item{
		Name: "ignored.toml",
		Fields: []types.CustomField{
			{Name: types.PKVNoteStrategyFieldName, Value: types.NoteStrategyMiseConfD},
			{Name: types.PKVNoteTargetFieldName, Value: " .config/mise/conf.d/custom.toml "},
		},
	}
	got, err := resolveNoteTarget(item, "team-a")
	if err != nil {
	...
	if got != ".config/mise/conf.d/custom.toml" {
		t.Fatalf("resolveNoteTarget() = %q", got)
	}
}

func TestResolveNoteTargetRejectsInvalidMiseConfDTarget(t *testing.T) {
	item := types.Item{
		Name: "tool.toml",
		Fields: []types.CustomField{
			{Name: types.PKVNoteStrategyFieldName, Value: types.NoteStrategyMiseConfD},
			{Name: types.PKVNoteTargetFieldName, Value: "mise.local.toml"},
		},
	}
	_, err := resolveNoteTarget(item, "team-a")
	if err == nil {
		t.Fatal("expected error")
	}
	if !strings.Contains(err.Error(), types.PKVNoteTargetFieldName) {
		t.Fatalf("unexpected error: %v", err)
	}
}

func TestSyncFolderWritesMiseConfDFragment(t *testing.T) {
	dir := t.TempDir()
	st := &state.State{}
	syncer := NewSyncer(st)
	...
	count, err := syncer.SyncFolder([]types.Item{item}, dir, "team-a")
	if err != nil {
		t.Fatalf("SyncFolder() error = %v", err)
	}
	if count != 1 {
	...
	path := filepath.Join(dir, ".config", "mise", "conf.d", "pkv-team-a-mise-ruby.toml")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read fragment: %v", err)
	}
	if string(data) != item.Notes {
		t.Fatalf("fragment content = %q", string(data))
	}
	if len(st.Notes) != 1 {
		t.Fatalf("state notes = %d, want 1", len(st.Notes))
	}
	if st.Notes[0].FileName != item.Name {
		t.Fatalf("tracked source name = %q, want %q", st.Notes[0].FileName, item.Name)
	}
	if st.Notes[0].FilePath != path {
		t.Fatalf("tracked file path = %q, want %q", st.Notes[0].FilePath, path)
	}
...
func TestSyncFolderMigratesTrackedNoteWhenStrategyChangesTarget(t *testing.T) {
	dir := t.TempDir()
	oldPath := filepath.Join(dir, "mise-ruby")
	if err := os.WriteFile(oldPath, []byte("[tools]\nruby='3.2.2'\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	_, err := syncer.SyncFolder([]types.Item{item}, dir, "team-a")
	if err != nil {
		t.Fatalf("SyncFolder() error = %v", err)
	}
	if _, err := os.Stat(oldPath); !os.IsNotExist(err) {
	...
	newPath := filepath.Join(dir, ".config", "mise", "conf.d", "pkv-team-a-mise-ruby.toml")
	...
	if entry.FilePath != newPath {
		t.Fatalf("tracked file path = %q, want %q", entry.FilePath, newPath)
	}
}
